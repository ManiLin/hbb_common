...
impl Config {
    ...
    fn load() -> Config {
        ...
        // Fork fix: ids auto-generated by the legacy MAC-only algorithm collide
        // with upstream RustDesk installed on the same host. Regenerate them
        // with the salted fork algorithm. `auto_id_from_mac(.., "RustDesk", ..)`
        // reproduces the legacy value regardless of the current APP_NAME.
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        if !config.id.is_empty() {
            if let Ok(Some(ma)) = mac_address::get_mac_address() {
                let legacy = Config::auto_id_from_mac(&ma.bytes(), "RustDesk", "").to_string();
                if config.id == legacy {
                    for _ in 0..3 {
                        if let Some(id) = Config::gen_id() {
                            log::info!("Migrated colliding legacy auto id {} to {}", config.id, id);
                            config.id = id;
                            store = true;
                            break;
                        }
    ...
    /// Derives the numeric id from a MAC address.
    ///
    /// For upstream RustDesk (`APP_NAME == "RustDesk"`) this is the legacy
    /// MAC-only value. Fork builds mix in the app name and desktop UI flavor so
    /// that a fork installed next to upstream RustDesk on the same host does
    /// not end up with the same id and fight over it on the rendezvous server.
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn auto_id_from_mac(bytes: &[u8], app_name: &str, flavor: &str) -> u32 {
        if app_name.eq_ignore_ascii_case("RustDesk") {
            let mut id = 0u32;
            for x in &bytes[2..] {
                id = (id << 8) | (*x as u32);
            }
        ...
        u32::from_be_bytes([digest[0], digest[1], digest[2], digest[3]]) & 0x1FFFFFFF
    }
    ...
    fn get_auto_id() -> Option<String> {
        ...
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        {
            if let Ok(Some(ma)) = mac_address::get_mac_address() {
                let app_name = APP_NAME.read().unwrap().clone();
                let id = Self::auto_id_from_mac(
                    &ma.bytes(),
                    &app_name,
                    DEFAULT_DESKTOP_UI_FLAVOR_FROM_BUILD,
                );
...
pub mod keys {
    ...
    pub const KEYS_SETTINGS: &[&str] = &[
        ...
        OPTION_INVENTORY_UPDATE_META_URL,
    ];
...
mod tests {
    ...
    #[test]
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    fn test_auto_id_is_fork_specific() {
        let mac = [0x00u8, 0x11, 0x22, 0x33, 0x44, 0x55];
        let upstream = Config::auto_id_from_mac(&mac, "RustDesk", "");
        // Legacy MAC-only value is preserved for upstream RustDesk.
        assert_eq!(upstream, 0x02334455);
        let fork = Config::auto_id_from_mac(&mac, "TnursRemoteDesk", "");
        let fork_cash = Config::auto_id_from_mac(&mac, "TnursRemoteDesk", "cashdesk");
        assert_ne!(upstream, fork);
        assert_ne!(fork, fork_cash);
        // Deterministic for the same inputs.
        assert_eq!(fork, Config::auto_id_from_mac(&mac, "TnursRemoteDesk", ""));
    }
